...
df = pd.read_csv(
    "C:\\Users\\User\\Desktop\\Python\\Earthquake-prediction-Maksimov\\Earthquake_Data.csv",
    delimiter=r"\s+",
)
...
scaler = MinMaxScaler()
df[["Latitude(deg)", "Longitude(deg)"]] = scaler.fit_transform(
    df[["Latitude(deg)", "Longitude(deg)"]]
)
